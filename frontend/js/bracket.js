...
function formatGracketData(bagan) {
    ...
    for(let i=0; i<4; i++) {
        ...
        r1.push([
            {name: match.tim_A.nama, id: match.tim_A.id || "r1a"+i},
            {name: match.tim_B.nama, id: match.tim_B.id || "r1b"+i}
        ]);
    }
    ...
    for(let i=0; i<2; i++) {
        let match = bagan["Semi Final"] && bagan["Semi Final"][i] 
                    ...
                    : {tim_A:{nama:"TBD"}, tim_B:{nama:"TBD"}};
        r2.push([
            {name: match.tim_A.nama, id: match.tim_A.id || "r2a"+i},
            {name: match.tim_B.nama, id: match.tim_B.id || "r2b"+i}
        ]);
    }
    ...
    let matchFinal = bagan["Final"] && bagan["Final"][0] 
                     ...
                     : {tim_A:{nama:"TBD"}, tim_B:{nama:"TBD"}};
    r3.push([
        {name: matchFinal.tim_A.nama, id: matchFinal.tim_A.id || "r3a"},
        {name: matchFinal.tim_B.nama, id: matchFinal.tim_B.id || "r3b"}
    ]);

    // Round 4: Pemenang Juara 1
    let r4 = [];
    let champ = {name: "CHAMPION", id: "champ"};
    
    if (bagan["Final"] && bagan["Final"][0] && bagan["Final"][0].pemenang_id) {
        let f = bagan["Final"][0];
        if (f.pemenang_id === f.tim_A.id) champ = {name: f.tim_A.nama, id: f.tim_A.id};
        else if (f.pemenang_id === f.tim_B.id) champ = {name: f.tim_B.nama, id: f.tim_B.id};
    }
